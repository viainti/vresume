...
import { initialResumeStateEn, initialResumeStateEs } from "@/config/initialResumeData";
import { DEFAULT_TEMPLATES } from "@/config";

const getBaseResumeState = () => {
  if (typeof window !== "undefined") {
    const locale = document.cookie
      .split("; ")
      .find((row) => row.startsWith("NEXT_LOCALE="))
      ?.split("=")[1] || "es";
    return locale === "en" ? initialResumeStateEn : initialResumeStateEs;
  }
  return initialResumeStateEs;
};
...
export const createResumeFromAIResult = (result: any, fileName: string) => {
  ...
  const baseState = getBaseResumeState();
